...
const Coupons = () => {
    ...
    function MyVerticallyCenteredModal(props) {
        ...
        const [ expireDate , setExpirationDate] = useState("")
      ...
      const postHandler = async (e) => {
        ...
        try {
          const { data } = await axios.post(
            "https://52pv9t2fl3.execute-api.ap-south-1.amazonaws.com/dev/api/v1/coupon",
            {code , expireDate , activateDate , discount}
          );
          console.log(data);
          toast.success("Added");
          fetchData();
          ...
          <Modal.Body>
            ...
            >
              ...
              <Form.Group className="mb-3" controlId="formBasicPassword">
                ...
                <Form.Control type="date"   onChange={(e) => setActivationDate(e.target.value)}/>
              </Form.Group>
    ...
    const deleteHandler = async (id) => {
        try{
            const { data } = await axios.delete(`https://52pv9t2fl3.execute-api.ap-south-1.amazonaws.com/dev/api/v1/coupon/${id}`)
            console.log(data)
            fetchData()
              ...
              <tbody>
                {data?.map((i, index) => (
                  <tr key={index}>
                    ...
                    <td>{i.discount}</td>
                    <td>{i.expireDate}</td>
                    <td>{i.activateDate}</td>
                    <td>{i.count}</td>
                    <td>
                      <i class="fa-solid fa-trash" onClick={() => deleteHandler(i._id)}></i>
                    </td>
                  </tr>
                ))}
              </tbody>
