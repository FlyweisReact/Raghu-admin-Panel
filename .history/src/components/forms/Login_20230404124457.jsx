...
const Login = () => {
  ...
  const login = async (e) => {
    ...
    setLoading(true);
    try {
      const {data } = await axios.post("https://52pv9t2fl3.execute-api.ap-south-1.amazonaws.com/dev/api/v1/admin/login" , {email , password})
      console.log(data)
      navigate("/dashboard");
      ...
      <div className="w-full h-screen flex flex-col justify-center items-center bg-slate-100">
        ...
        >
          ...
          <section className="py-7 space-y-6">
            ...
            <button
              type="submit"
              className="py-2 cursor-pointer tracking-wider]\flex justify-center items-center w-full rounded-md font-medium"
