  ...
  const AddWalletHandler = async (e) => {
    ...
    try {
      if (dropDown === "removebalance") {
        const data = await axios.post(
          `httpec2-15-206-210-177.ap-south-1.compute.amazonaws.com:5002/api/wallet/addandremoveMoneyinadmin/${id}`,
          {
      ...
      if (dropDown === "addbalance") {
        const data = await axios.post(
          `ec2-15-206-210-177.ap-south-1.compute.amazonaws.com:5002/api/wallet/addandremoveMoneyinadmin/${id}`,
          {
            addbalance: amount,
          }
        );
